...
public class ArmHook extends GhidraScript {
    static final long DISPATCH = 0x0C0C9E60L;   // pad-function dispatch
    static final long ARM07    = 0x0C0CA010L;   // code 0x07 arm
    static final long LIT      = 0x0C0CA130L;   // arm 0x07 jmp-target literal (-> 0x0C0C747C)
    ...
    public void run() throws Exception {
        ...
        runDispatch("PATCHED dispatch, padFunc=0x07 -> stop-all", true, 0x07);
        ...
        cfReturn("PATCHED padFunc=0x07 CONTROL-FLOW-ONLY -> clean return to caller", 0x07);
        // Regression: UNPATCHED, code 0x07 -> normal arm (jmp 0x0C0C747C, a display refresh; stubbed).
        runDispatch("UNPATCHED dispatch, padFunc=0x07 -> normal (control)", false, 0x07);
        // Regression: PATCHED, code 0x08 (click) -> must still take its OWN arm, not stop-all.
        runDispatch("PATCHED dispatch, padFunc=0x08 -> click arm unaffected", true, 0x08);

        P("\n######## SUMMARY ########");
        for(Map.Entry<String,String> e: results.entrySet()) P("  "+e.getKey()+"  ->  "+e.getValue());
        PrintWriter pw=new PrintWriter("/Users/velvetmotion/Desktop/DTX-M12/firmware/armhook_emu.txt");
        pw.print(log); pw.close(); P("\nwrote armhook_emu.txt");
    }
