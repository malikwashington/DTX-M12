...
public class Ladder extends GhidraScript {
  ...
  void info(long fe){
    ...
    println("\n###### addr 0x"+Long.toHexString(fe)+" in "+(f==null?"NO FUNC":(f.getName()+" @"+f.getEntryPoint()+" body .."+f.getBody().getMaxAddress())));
    if(f==null) return;
    DecompileResults r=dec.decompileFunction(f,90,mon);
    if(r!=null&&r.decompileCompleted()) println(r.getDecompiledFunction().getC());
    else println("(decomp fail)");
    println("---- DISASM "+f.getEntryPoint()+".."+f.getBody().getMaxAddress()+" ----");
